...
switch (player_mode) {
    ...
    case 0:
		...
		if place_meeting_3d(x,y,z,par_atk) && !invincible {
			invincible	= true
			alarm[0]	= 60
			shake	= 10
			z--
...
spd_push[hh]	= lerp(spd_push[hh],0,.1)
spd_push[vv]	= lerp(spd_push[vv],0,.1)
...
if dice_time < 60*10 {
	...
	instance_create_depth(x,y,depth,o_throw, {
		spd		: [lengthdir_x(interpol(DIS_R_CENTER,limit_sup[0],limit_inf[0],_lim[hh],_lim[vv]),_dir),lengthdir_y(interpol(DIS_R_CENTER,limit_sup[0],limit_inf[0],_lim[hh],_lim[vv]),_dir)],		
